...
class Bot {
    ...
    public function DefaultBot() {
        ...
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/botId',
    ...
    public function Login($username, $password) {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/login',
            CURLOPT_RETURNTRANSFER => 1,
    ...
    public function GetInstances() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/instances',
            CURLOPT_RETURNTRANSFER => 1,
    ...
    public function GetFiles() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/files',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetInstanceLog() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/log',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetUsers() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/users',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetPlaylists() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/playlists',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
    ...
    public function GetRadioStations() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/stations',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetInfo() {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/info',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/volume/set/'.$volume,
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/play/byId/'.$uuid,
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId, 'trackid' => $uuid)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/playUrl?url='.$urlplay.'&title='.$title,
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId, 'trackid' => $uuid)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/playPrevious',
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/playNext',
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/repeat/'.$value,
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/shuffle/'.$value,
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId, 'status' => $value)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/stop',
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/volume/up',
            CURLOPT_POSTFIELDS => json_encode(array('instanceID' => $instanceId)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/volume/down',
            CURLOPT_POSTFIELDS => json_encode(array('instanceId' => $instanceid)),
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/status',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
	...
	$ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/jobs',
	...
	$ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/upload',
	...
	if($instanceId == NULL){ $instanceId = $this->selected; }
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/i/'.$instanceId.'/settings',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetPlaylistTracks($playlistId) {
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => 'https://'.$this->botHost.':'.$this->botPort.'/api/v1/bot/playlists/'.$playlistId.'',
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_HTTPHEADER => array('Authorization: bearer '.$this->token)
        ));
        $data = curl_exec($ch);
    ...
    public function GetThumbnail($thumbnail) {
        return 'https://'.$this->botHost.':'.$this->botPort.'/cache/'.$thumbnail;
    }
}
?>